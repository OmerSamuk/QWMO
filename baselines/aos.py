...
class AOS:
    ...
    def run(self):
        ...
        while self.fes_count < self.max_fes:
            max_lay = int(self.rng.integers(1, self.layer_number + 1))
            ...
            for i in range(max_lay):
                ...
                for j in range(len(pop_a)):
                    if self.rng.random() > self.foton_rate:
                        if cost_a[j] > energy:
                            ir = self.rng.uniform(0, 1, 2)
                            jr = self.rng.uniform(0, 1, self.dimension)
                            x_old = pop_a[j]
                        ...
                        pop_b[j] = self.rng.uniform(self.lower_bound, self.upper_bound, self.dimension)
