...
class ASO:
    ...
    def compute_acceleration(self, iteration, max_iteration):
        ...
        for i in range(self.population_size):
            ...
            for k in range(Kbest):
                ...
                E += self.rng.random(self.dimension) * potential * \
    ...
    def run(self):
        ...
        self.velocities = self.rng.uniform(self.lower_bound, self.upper_bound,
                                           (self.population_size, self.dimension))
        ...
        for iteration in range(2, max_iteration + 1):
            ...
            self.velocities = self.rng.random((self.population_size, self.dimension)) * self.velocities + acceleration
            self.positions = self.positions + self.velocities

            for i in range(self.population_size):
                out_of_bounds = (self.positions[i] > self.upper_bound) | (self.positions[i] < self.lower_bound)
                if np.any(out_of_bounds):
                    self.positions[i, out_of_bounds] = self.rng.uniform(self.lower_bound, self.upper_bound,
            ...
            if min_fitness < self.best_fitness:
                ...
                r = self.rng.integers(0, self.population_size)
